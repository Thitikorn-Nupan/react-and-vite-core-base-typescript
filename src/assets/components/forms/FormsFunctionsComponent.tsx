        ...
        <div className={"container w-75 mt-4"}>
            <form className={"form-control p-2"}>
                <h3 className={"alert alert-primary p-2"}>
                    Work With Form Inside Function (name and id on tags doesn't matter)
                    <br/>*** Handle Functions on tag input *** Submit by button
                </h3>
                <div className="mb-3">
                    Email
                    ...
                    <strong className={(email.length <= 5) ? "text-danger mt-1" : "text-success mt-1"}>
                        {email.length <= 5 ? "length is not good" : "length is enough"}
                    </strong>
                </div>
                <div className="mb-3">
                    Username
                    ...
                    <strong className={(username.length <= 5) ? "text-danger mt-1" : "text-success mt-1"}>
                        {username.length <= 5 ? "length is not good" : "length is enough"}
                    </strong>
                </div>
...
const Form2 = () => {
    ...
    const languages: string[] = ["Java", "C,C++", "Php"]
    /**
     // these function for changing language1,2,3 ** just switch ** work only true and false
     const handleLanguage1Click = () => {
     // why ! , if we click we have to change default the first way change by ! it means !true = false
    ...
    return (
        <div className={"container w-50 mt-4"}>
            <form className={"form-control"}>
                <h3 className={"alert alert-primary"}>which language you know about programming ?</h3>
...
// ** Multiple inputs , setInputs ** Cool !
interface MyObject { // Define the type for your object
    rid: number;
    codeName: string,
    price: number,
}
const Form3MultipleInputs = () => {
    ...
    const handleLoadEachInputToInputsArray = (event: FormEvent<HTMLInputElement>) => {
        ...
        setInputs(values => ({
                ...values,
                [name]: value // set dynamic key , value
            })
        )
    }
    ...
    return (
        <>
            ...
            {/***  */}
            <div className={"container w-75 mt-4"}>
                <table className="table mt-3 " style={{margin: "0 auto", "cursor": "pointer"}}>
                    ...
                    <tbody>
                    {
                        robots.map((robot : Robot, index : number) => (
                            <tr key={robot.rid}
                                ...
                                <td>{robot.price}</td>
                                <td>{robot.active && "true"} {!robot.active && "false"}</td>
                            </tr>
                        ))
                    }
                    </tbody>
...
const Form5setStateAsObject = () => {
    ...
    return (
        <div className={"container w-50 mt-4"}>
            <form className={"form-control p-2"}>
                <h3 className={"alert alert-primary p-2"}>*** The Way to Updating Objects and Arrays in State</h3>
                <div className="mb-3">
                    Brand
